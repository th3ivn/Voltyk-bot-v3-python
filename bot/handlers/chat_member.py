...
from aiogram.types import ChatMemberUpdated, InlineKeyboardMarkup, User
...
async def _handle_bot_added(
    ...
    """Handle the bot being promoted to administrator in a channel."""
...
async def _handle_bot_removed(
    ...
    """Handle the bot being removed from (or losing admin in) a channel."""
...
async def handle_chat_member(event: ChatMemberUpdated, session: AsyncSession) -> None:
    ...
    if new_status in ("administrator", "creator") and old_status in ("left", "kicked", "member"):
        ...
        await _handle_bot_removed(event, session, channel_id=channel_id, channel_title=channel_title)
